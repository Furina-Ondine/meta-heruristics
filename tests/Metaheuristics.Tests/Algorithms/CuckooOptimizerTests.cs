...
/// <summary>验证布谷鸟算法的配置、尺度、工作区、确定性和 Core 集成行为。</summary>
public sealed class CuckooOptimizerTests
{
    [Xunit.Fact]
    public void RunEvaluatesInitialPopulationAndUsesCoreOrdering()
    {
        var objective = new FirstCoordinateObjective();
        var optimizer = new CuckooOptimizer(
            new SequenceInitializer(-1, 1, 2),
            new CuckooOptimizerOptions { PopulationSize = 3, LevyCandidateCount = 1 });
    ...
    [Xunit.Fact]
    public void UserConfiguredLevyCandidateCountControlsRepairWork()
    {
        var repair = new RecordingRepair();
        var optimizer = new CuckooOptimizer(
            new SequenceInitializer(3, 2, 1),
            new CuckooOptimizerOptions
            {
                PopulationSize = 3,
                LevyCandidateCount = 1,
                AbandonmentRate = 0,
                BaseLevyScale = 0.1,
                AbandonmentPerturbationScale = 0,
            });
    ...
    [Xunit.Fact]
    public void SingleNestWithFullAbandonmentCompletesWithoutRetryLoop()
    {
        var optimizer = new CuckooOptimizer(
            new SequenceInitializer(1),
            new CuckooOptimizerOptions
            {
                PopulationSize = 1,
                LevyCandidateCount = 1,
        ...
        Xunit.Assert.Equal(3, result.Evaluations);
        Xunit.Assert.True(double.IsFinite(result.BestEvaluation.Objective));
    }

    [Xunit.Fact]
    public async Task RunIsReproducibleAndIndependentInstancesRemainIsolated()
    {
        var options = new CuckooOptimizerOptions
        {
            PopulationSize = 16,
            LevyCandidateCount = 6,
            BaseLevyScale = 0.4,
            AbandonmentPerturbationScale = 0.1,
        };
    ...
    public void RunImprovesSphereFixture()
    {
        ...
        {
            PopulationSize = 24,
            LevyCandidateCount = 8,
            BaseLevyScale = 0.3,
            AbandonmentPerturbationScale = 0.1,
            StepDecayIterations = 80,
        };
    ...
    {
        Xunit.Assert.Throws<ArgumentNullException>(() => new CuckooOptimizer(null!));
        Xunit.Assert.Throws<ArgumentOutOfRangeException>(
            () => CreateOptimizer(new CuckooOptimizerOptions { PopulationSize = 0 }));
        Xunit.Assert.Throws<ArgumentOutOfRangeException>(
            () => CreateOptimizer(new CuckooOptimizerOptions { AbandonmentRate = 1.1 }));
        Xunit.Assert.Throws<ArgumentOutOfRangeException>(
            () => CreateOptimizer(new CuckooOptimizerOptions { LevyExponent = 2 }));
        Xunit.Assert.Throws<ArgumentOutOfRangeException>(
            () => CreateOptimizer(new CuckooOptimizerOptions { GaussianScale = double.NaN }));
        Xunit.Assert.Throws<ArgumentOutOfRangeException>(
            () => CreateOptimizer(new CuckooOptimizerOptions { AbandonmentPerturbationScale = -1 }));
        Xunit.Assert.Throws<ArgumentException>(
            () => CreateOptimizer(new CuckooOptimizerOptions { PopulationSize = 2, LevyCandidateCount = 3 }));
        Xunit.Assert.Throws<ArgumentOutOfRangeException>(
            () => CreateOptimizer(new CuckooOptimizerOptions { StepDecayIterations = 0 }));
    }

    private static CuckooOptimizer CreateOptimizer(CuckooOptimizerOptions? options = null)
    {
        return new CuckooOptimizer(new RandomPositionInitializer(), options);
    }
    ...
    private static double[][] GetVectors(Array population)
    {
        ...
        for (var index = 0; index < population.Length; index++)
        {
            var state = population.GetValue(index)
                ?? throw new InvalidOperationException("The cuckoo state was null.");
